...
public class Day2 : IChallenge
{
    ...
    private static bool IsSafe(List<int> report)
    {
        ...
        // Get differences between adjacent levels
        var differences = new int[report.Count - 1];
        ...
        var isIncreasing = differences.All(diff => diff > 0);
        var isDecreasing = differences.All(diff => diff < 0);
        ...
        return differences.All(diff => Math.Abs(diff) >= 1 && Math.Abs(diff) <= 3);
    }
